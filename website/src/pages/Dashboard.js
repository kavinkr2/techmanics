...
import { Anchor, Globe, Loader2, Package, Ship, TrendingUp, AlertTriangle, ExternalLink, Download, RefreshCw, } from "lucide-react";
...
function FreightRateTrendChart({ data, loading }) {
    const grouped = useMemo(() => {
        const byRoute = new Map();
        data.forEach((r) => {
            if (!byRoute.has(r.route))
                byRoute.set(r.route, { route: r.route, rates: [] });
    ...
    if (loading) {
        return (_jsx(Card, { className: "p-5", children: _jsx("div", { className: "flex items-center justify-center h-64 text-text-muted", children: _jsx(Loader2, { className: "h-6 w-6 animate-spin" }) }) }));
    }
    if (!chartData.length) {
        return (_jsx(Card, { className: "p-5", children: _jsx("div", { className: "flex items-center justify-center h-64 text-text-muted", children: _jsx("p", { children: "No freight rate data available" }) }) }));
    }
    return (_jsxs(Card, { className: "p-5", children: [_jsxs("div", { className: "flex items-center justify-between mb-4", children: [_jsxs("div", { children: [_jsx("h3", { className: "text-lg font-medium text-text-primary", children: "Freight Rate Trends" }), _jsx("p", { className: "text-sm text-text-secondary/60", children: "Average rate (USD/tonne) by route" })] }), _jsxs(Badge, { variant: "default", children: [chartData.length, " routes"] })] }), _jsx("div", { className: "h-[280px]", children: _jsx(ResponsiveContainer, { width: "100%", height: "100%", children: _jsxs(BarChart, { data: chartData, margin: { top: 10, right: 24, left: 0, bottom: 0 }, children: [_jsx(CartesianGrid, { strokeDasharray: "3 3", stroke: "oklch(0.9 0.005 260)", vertical: false }), _jsx(XAxis, { dataKey: "route", tick: { fill: "oklch(0.45 0.01 260)", fontSize: 10 }, tickLine: false, axisLine: false }), _jsx(YAxis, { tick: { fill: "oklch(0.45 0.01 260)", fontSize: 11 }, tickLine: false, axisLine: false, tickFormatter: (v) => `$${v}` }), _jsx(Tooltip, { contentStyle: {
                                    backgroundColor: "oklch(0.15 0.02 260 / 0.9)",
                                    border: "1px solid oklch(0.9 0.005 260)",
                                    borderRadius: "0.75rem",
                                    color: "oklch(0.98 0 0)",
                                }, formatter: (value) => [`$${value.toFixed(1)}/t`, "Rate"] }), _jsx(Bar, { dataKey: "avg", radius: [4, 4, 0, 0], children: chartData.map((_, i) => (_jsx(Cell, { fill: colors[i % colors.length] }, `cell-${i}`))) })] }) }) })] }));
}
function VesselPositionsPanel({ data, loading }) {
    ...
    if (loading) {
        return (_jsx(Card, { className: "p-5 mb-6", children: _jsx("div", { className: "flex items-center justify-center h-40 text-text-muted", children: _jsx(Loader2, { className: "h-5 w-5 animate-spin" }) }) }));
    }
    return (_jsxs(Card, { className: "p-5 mb-6", children: [_jsxs("div", { className: "flex items-center justify-between mb-4", children: [_jsxs("div", { children: [_jsx("h3", { className: "text-lg font-medium text-text-primary", children: "Live Vessel Positions" }), _jsxs("p", { className: "text-sm text-text-secondary/60", children: [total, " vessels tracked globally"] })] }), _jsxs(Badge, { variant: "default", children: [byType.length, " types"] })] }), _jsxs("div", { className: "space-y-3", children: [data.slice(0, 6).map((v) => (_jsxs("div", { className: "flex items-center gap-3 p-3 rounded-[10px] bg-surface-muted", children: [_jsx("div", { className: "w-10 h-10 rounded-full bg-accent-light flex items-center justify-center shrink-0", children: _jsx(Ship, { className: "h-5 w-5 text-accent" }) }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("p", { className: "font-medium text-text-primary", children: v.name }), _jsxs("p", { className: "text-xs text-text-secondary", children: [v.type, " \u00B7 ", v.destination || "At sea"] })] }), _jsxs("div", { className: "text-right", children: [_jsxs("p", { className: "text-sm font-medium text-text-primary", children: [v.speed, " kn"] }), _jsxs("p", { className: "text-xs text-text-secondary", children: ["ETA: ", v.eta] })] })] }, v.mmsi))), data.length === 0 && (_jsx("p", { className: "text-center text-text-muted py-6", children: "No vessel data available" }))] }), total > 0 && (_jsxs("div", { className: "mt-4 pt-4 border-t border-border/30", children: [_jsx("p", { className: "text-xs text-text-secondary uppercase tracking-wider mb-2", children: "By Vessel Type" }), _jsx("div", { className: "space-y-2", children: byType.slice(0, 5).map(([type, count]) => (_jsxs("div", { className: "flex items-center gap-3", children: [_jsx("span", { className: "w-24 text-xs text-text-secondary", children: type }), _jsx("div", { className: "flex-1 h-2 bg-surface-2/40 rounded-full overflow-hidden", children: _jsx("div", { className: "h-full bg-accent/50 rounded-full transition-all", style: { width: `${(count / total) * 100}%` } }) }), _jsx("span", { className: "w-12 text-right text-xs font-mono text-text-primary", children: count })] }, type))) })] }))] }));
}
...
export default function DashboardPage() {
    ...
    const load = async () => {
        try {
            setError(null);
            const [h, f, bdi, rates, congestion, vessels, alerts,] = await Promise.allSettled([
                api.health(),
                api.forecast(90),
